...
|---|------|----------------|---------|
| 1 | `crates/node/Cargo.toml` | `version = "x.y.z"` (read by `relay-node --version` via `env!("CARGO_PKG_VERSION")`) | 1.0.3 |
| 2 | `scripts/relay-node-install.sh` | `SCRIPT_VERSION="x.y.z"` (decides which release binary to download) | 1.0.3 |
| 3 | `docker-compose.release.yaml` | both image tags: `ghcr.io/moeshinx/relay-panel-panel:x.y.z` AND `.../relay-panel-node:x.y.z` | 1.0.3 |
| 4 | `crates/panel/src/config.rs` | `COMPILED_APP_VERSION` (the panel's own version, shown in the update-check UI). Overridable at runtime via the `APP_VERSION` env var. | 1.0.3 |
| 5 | `README.md` | the `**Version:** \`x.y.z\`` badge line | 1.0.3 |
| 6 | `README.md` | the `**当前版本：** \`x.y.z\`` badge line | 1.0.3 |
| 7 | `crates/panel/Cargo.toml` | `version = "x.y.z"` (the panel crate version). `release-check.sh` FAILs if it drifts. `Cargo.lock` carries it too, so run `cargo check` after bumping. | 1.0.3 |
...
```powershell
$v = '1.0.3'  # the version you are migrating FROM
